...
- **管理の効率化**: 画像ファイルを追加するだけで基本的なデータ登録が完了する運用フローを確立する。
...
- **同期**: Excel/CSV/TSV から `plants.json` を生成するスクリプトを利用可能。
...
│   └── images/       # 植物画像格納場所
├── scripts/
│   ├── json_to_excel.cjs # JSON → Excel
│   ├── sync_excel.cjs    # Excel → JSON
│   ├── sync_csv.cjs      # CSV → JSON
│   └── sync_tsv.cjs      # TSV → JSON
├── data/
│   └── plants.xlsx  # Excel管理用データ
├── src/
...
| フィールド名 | 型 | 説明 | 例 |
| --- | --- | --- | --- |
| `id` | String | ユニークID（画像ファイル名プレフィックス） | `"0001"` |
| `slug` | String | URL用スラッグ（通常IDと同じ） | `"0001"` |
| `title` | String | 表示名（カタカナ） | `"アダン"` |
| `description`| String | 説明文 | `"海岸近くに自生する..."` |
| `images` | Array<Object / String> | 画像パスの配列 | `[{ "src": "/images/0001_00.jpg" }]` |
...
2.  **データ更新**: `src/data/plants.json` を直接編集、もしくは Excel/CSV/TSV 同期で生成。
3.  **情報追記**: 必要に応じて `description` や `colors` を手動編集。
